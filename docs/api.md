...
| Param | Type | Description |
|---|---|---|
| `format` | string | Output format: `csv` or `json` (default: `csv`). CSV output returns a standard comma-separated text file with headers. JSON output returns an array of objects. |
...
|---|---|---|
| `format` | string | Output format: `csv` or `json` (default: `csv`). CSV output returns a standard comma-separated text file with headers. JSON output returns an array of objects. |
| `start` | string | Start timestamp (`YYYY-MM-DDTHH:MM`) |
| `end` | string | End timestamp (`YYYY-MM-DDTHH:MM`) |
| `src_ip` | string | Source IP filter |
| `dst_ip` | string | Destination IP filter |
